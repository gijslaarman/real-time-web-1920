...
function detectRoomInputChange() {
    const roomPinInput = document.querySelector('input[name=roomPin]')
...
function loginFormDetector() {
    ...
    loginForm.addEventListener('submit', function (e) {
        ...
        if (values.roomPin) {
            fetch(postURL, { method: 'POST' })
                ...
                .then(room => {
                    ...
                    if (room) {
                        ...
                        socketHandler.emit('join room', values)
                    } else {
            ...
            socketHandler.emit('create new room', values)
        }
    })
}
...
function createShareablePinLink(pin) {
...
function makeGameRoom(pin, cards) {
    // Fill shareable pin link elements with this rooms pin.
    createShareablePinLink(pin)
    state.pin = pin

    // Remove the login overlay.
    document.querySelector('.login').remove()

    cardManager.generateCards(cards)
}

function setup() {
    checkForPinQuery()
    detectRoomInputChange()
    loginFormDetector()
}
export { makeGameRoom }
